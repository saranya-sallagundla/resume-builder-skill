...
- **2026-09-14 — Part 2 shipped.** Résumé room live: Build, Score &amp; X-Ray with a keyword placement map, Coach, Three Readers, Compare. Extractor no longer promotes generic verbs; length rule is now ≤ 2 pages measured at real Letter/A4 size. A half-built "copy share link" button was removed rather than shipped working-ish.
- **2026-09-14 — Part 3 skill bank lives in the page, not a data file.** The bank is strict JSON between `BANK-START`/`BANK-END` markers inside `web/index.html`, so the page stays a single file that works from `file://` with no fetch (CLAUDE.md rule 4), while `tests/test_web_page.py` still parses and schema-checks it without running any JavaScript. A future scheduled Action can read the same block to flag entries older than six months.
- **2026-09-14 — Questions are traced, never padded.** Every generated question names the JD line or résumé bullet it came from. When a job description is too thin to yield fifty, the room says so and shows the real number instead of inventing filler. Skills carry a *kind* (`tool` vs `practice`) because "when would you argue against using Observability?" is a nonsense question and "when would you argue against using Kubernetes?" is a good one.
- **2026-09-15 — Whether a provider is browser-callable cannot be decided from here.** Mode B assumed any free-tier provider would do. A first probe suggested Groq worked and Google AI Studio and OpenRouter did not, and that was briefly written into the UI as fact. A second probe the next day showed **Groq failing the identical test it had passed twice**, while the control request still returned 200 — almost certainly the provider rate-limiting repeated invalid keys. That is the lesson: **a browser reports "CORS failure" identically for "this API refuses browsers", "your network blocked it" and "you are being rate-limited"**, so no test run from one machine can settle it. The UI therefore asserts nothing about reachability; all providers are listed neutrally, **Test connection** is the source of truth for the person actually using it, and the failure message names all three possible causes. Groq stays the default only because it is free and fast, not because it is proven. Mode A (on-device, Part 6) avoids the question entirely, which raises its value.
- **2026-09-15 — The privacy page must not overpromise.** It claimed the PII strip removed "addresses"; it removes emails, phone numbers and links, and cannot remove names or employers. Wording corrected to match the code. Any future change to `stripPII` changes a privacy promise and must update that page in the same commit.
